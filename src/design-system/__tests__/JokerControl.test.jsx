...
describe('JokerControl', () => {
  it('renders the approved pill with star, Joker label and multiplier only when armed', () => {
    const inactive = renderToStaticMarkup(
      React.createElement(JokerPill, { statusLabel: 'Add joker', matchLabel: 'match 1', onClick: noop }),
    )
    ...
    const active = renderToStaticMarkup(
      React.createElement(JokerPill, {
        active: true,
        statusLabel: 'Joker applied',
        matchLabel: 'match 1',
        multiplier: 2,
        onClick: noop,
      }),
  ...
  it('renders disabled cap treatment as a real disabled pill', () => {
    const html = renderToStaticMarkup(
      React.createElement(JokerPill, {
        disabled: true,
        statusLabel: 'Joker limit reached',
        matchLabel: 'match 2',
        onClick: noop,
      }),
    )
  ...
  it('renders a five-dot joker meter with filled count and accessible text', () => {
    const html = renderToStaticMarkup(
      React.createElement(JokerMeter, { value: 3, max: 5, multiplier: 2, label: 'group jokers selected' }),
    )
