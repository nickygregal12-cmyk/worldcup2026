...
const SECTION_TABS = {
  ...
  matches: [
    { key: 'matches', label: 'All fixtures', superOnly: true },
    { key: 'kofixtures', label: 'KO fixture health', superOnly: true },
    { key: 'ko', label: 'KO match control', superOnly: true },
  ],
  users: [
...
function MatchFixtureEditor({ match, teams, groups, venues, onClose, onSaved, setActionResult, logAudit }) {
  ...
  const [form, setForm] = useState({
    ...
    aet_home_score: match.aet_home_score ?? '',
    ...
    first_goal_band: match.first_goal_band || '',
  ...
  const save = async () => {
    if (!form.home_team_id || !form.away_team_id) {
      setActionResult('Choose both teams before saving the fixture.')
      return
    }
    if (form.home_team_id === form.away_team_id) {
    ...
    const payload = {
      ...
      status: form.status,
      live_minute: nullableNumber(form.live_minute),
      home_score: nullableNumber(form.home_score),
      away_score: nullableNumber(form.away_score),
      aet_home_score: nullableNumber(form.aet_home_score),
      aet_away_score: nullableNumber(form.aet_away_score),
      penalty_home_score: nullableNumber(form.penalty_home_score),
      penalty_away_score: nullableNumber(form.penalty_away_score),
      outcome_type: form.outcome_type || null,
    ...
    if (error) {
      setActionResult(`Fixture save failed: ${error.message}`)
      setSaving(false)
      return
    }
    await logAudit('EDIT_MATCH_FIXTURE', { match_id: match.id, match_number: payload.match_number, changes: payload })
    setActionResult(`Match ${payload.match_number} fixture details saved.`)
    await onSaved()
    setSaving(false)
    onClose()
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.58)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="card" style={{ width: 'min(760px, 100%)', maxHeight: '92dvh', overflowY: 'auto', borderRadius: '18px 18px 0 0', padding: '18px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
          <div><div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase' }}>Fixture control</div><h2 style={{ fontSize: '20px', margin: '3px 0 0' }}>Edit Match {match.match_number}</h2></div>
          <button className="btn btn-secondary btn-sm" onClick={onClose}>Close</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '12px' }}>
          <label><span className="admin-field-label">Match number</span><input className="input" style={fieldStyle} type="number" value={form.match_number} onChange={e => update('match_number', e.target.value)} /></label>
          <label><span className="admin-field-label">Stage</span><select className="input" style={fieldStyle} value={form.stage} onChange={e => update('stage', e.target.value)}>{['group','r32','r16','qf','sf','3rd','final'].map(v => <option key={v} value={v}>{v === 'group' ? 'Group stage' : v.toUpperCase()}</option>)}</select></label>
          <label><span className="admin-field-label">Group</span><select className="input" style={fieldStyle} disabled={form.stage !== 'group'} value={form.group_id} onChange={e => update('group_id', e.target.value)}><option value="">No group</option>{groups.map(g => <option key={g.id} value={g.id}>Group {g.name}</option>)}</select></label>
          <label><span className="admin-field-label">Status</span><select className="input" style={fieldStyle} value={form.status} onChange={e => update('status', e.target.value)}>{['scheduled','live','paused','completed','postponed','cancelled'].map(v => <option key={v} value={v}>{v[0].toUpperCase()+v.slice(1)}</option>)}</select></label>
          <label><span className="admin-field-label">Home team</span><select className="input" style={fieldStyle} value={form.home_team_id} onChange={e => update('home_team_id', e.target.value)}><option value="">Choose team</option>{teams.map(t => <option key={t.id} value={t.id}>{t.flag_emoji || ''} {t.name}</option>)}</select></label>
          <label><span className="admin-field-label">Away team</span><select className="input" style={fieldStyle} value={form.away_team_id} onChange={e => update('away_team_id', e.target.value)}><option value="">Choose team</option>{teams.map(t => <option key={t.id} value={t.id}>{t.flag_emoji || ''} {t.name}</option>)}</select></label>
          <label><span className="admin-field-label">Stadium</span><select className="input" style={fieldStyle} value={form.venue_id} onChange={e => update('venue_id', e.target.value)}><option value="">Choose stadium</option>{venues.map(v => <option key={v.id} value={v.id}>{v.name || v.stadium_name || v.city} · {v.city}{v.country ? `, ${v.country}` : ''}</option>)}</select></label>
          <label><span className="admin-field-label">Kickoff date and time</span><input className="input" style={fieldStyle} type="datetime-local" value={form.kickoff_time} onChange={e => update('kickoff_time', e.target.value)} /></label>
          <label><span className="admin-field-label">Live minute</span><input className="input" style={fieldStyle} type="number" min="0" max="130" value={form.live_minute} onChange={e => update('live_minute', e.target.value)} /></label>
          <label><span className="admin-field-label">Outcome type</span><select className="input" style={fieldStyle} value={form.outcome_type} onChange={e => update('outcome_type', e.target.value)}><option value="90mins">90 minutes</option><option value="aet">After extra time</option><option value="pens">Penalties</option></select></label>
          <label><span className="admin-field-label">First goal band</span><select className="input" style={fieldStyle} value={form.first_goal_band} onChange={e => update('first_goal_band', e.target.value)}><option value="">Not set</option><option value="0-15">0–15 mins</option><option value="16-30">16–30 mins</option><option value="31-45">31–45 mins</option><option value="46-60">46–60 mins</option><option value="61-75">61–75 mins</option><option value="76-90">76–90 mins</option><option value="no_goal">No goal</option></select></label>
          <label><span className="admin-field-label">External API match ID</span><input className="input" style={fieldStyle} value={form.external_match_id} onChange={e => update('external_match_id', e.target.value)} /></label>
        </div>
        <div style={{ marginTop: '14px', padding: '14px', background: 'var(--bg-secondary)', borderRadius: '12px' }}>
          <div style={{ fontWeight: 800, marginBottom: '10px' }}>Result details</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
            {[['home_score','Home score'],['away_score','Away score'],['aet_home_score','AET home'],['aet_away_score','AET away'],['penalty_home_score','Pens home'],['penalty_away_score','Pens away']].map(([key,label]) => <label key={key}><span className="admin-field-label">{label}</span><input className="input" type="number" min="0" max="30" value={form[key]} onChange={e => update(key, e.target.value)} /></label>)}
          </div>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: '9px', marginTop: '14px', fontSize: '13px', fontWeight: 700 }}><input type="checkbox" checked={form.use_manual_override} onChange={e => update('use_manual_override', e.target.checked)} /> Protect these details from API overwrite</label>
        <div style={{ display: 'flex', gap: '8px', marginTop: '18px', position: 'sticky', bottom: 0, background: 'var(--bg-card)', paddingTop: '12px' }}><button className="btn btn-primary" onClick={save} disabled={saving} style={{ flex: 1 }}>{saving ? 'Saving…' : 'Save fixture'}</button><button className="btn btn-secondary" onClick={onClose}>Cancel</button></div>
      </div>
      {fixtureEditorMatch && (
        <MatchFixtureEditor
