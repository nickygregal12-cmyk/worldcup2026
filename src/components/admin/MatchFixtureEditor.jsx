...
export default function MatchFixtureEditor({ match, teams, groups, venues, onClose, onSaved, setActionResult, logAudit }) {
  const [form, setForm] = useState({
    ...
    aet_away_score: match.aet_away_score ?? '', penalty_home_score: match.penalty_home_score ?? '', penalty_away_score: match.penalty_away_score ?? '',
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      penalty_home_score: num(form.penalty_home_score), penalty_away_score: num(form.penalty_away_score),
    ...
    <div style={{ position:'fixed', inset:0, zIndex:1400, background:'rgba(0,0,0,.62)', display:'flex', alignItems:'flex-end', justifyContent:'center' }} onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <div className="card" style={{ width:'min(820px,100%)', maxHeight:'94dvh', overflowY:'auto', borderRadius:'20px 20px 0 0', padding:'18px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))', gap:'12px' }}>
          ...
          <Field label="Outcome"><select className="input" value={form.outcome_type} onChange={e=>update('outcome_type',e.target.value)}><option value="90mins">90 minutes</option><option value="aet">Extra time</option><option value="pens">Penalties</option></select></Field>
        ...
        <div style={{ marginTop:'14px', padding:'14px', background:'var(--bg-secondary)', borderRadius:'12px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,minmax(0,1fr))', gap:'10px' }}>
            {[['home_score','Home score'],['away_score','Away score'],['aet_home_score','AET home'],['aet_away_score','AET away'],['penalty_home_score','Pens home'],['penalty_away_score','Pens away']].map(([k,l])=><Field key={k} label={l}><input className="input" type="number" min="0" max="30" value={form[k]} onChange={e=>update(k,e.target.value)} /></Field>)}
          </div>
        </div>
