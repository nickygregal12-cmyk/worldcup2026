...
export default function AccountDashboard({ client, reference, session, profile, profileName, setProfileName, emailAddress, busy, notice, dashboard, accountLifecycle, signOutConfirmOpen, setSignOutConfirmOpen, clearConfirmOpen, setClearConfirmOpen, displayNameConfirmOpen, setDisplayNameConfirmOpen, confirmDisplayNameChange, guestTransferRequested, onGuestTransferComplete, submitProfile, confirmSignOut, confirmClearPredictions, requestPasswordChange }) {
  ...
  return (
    <section className={styles.dashboard}>
      ...
      <Card className={`${styles.card} ${styles.dangerCard}`} as="section" aria-labelledby="account-danger-heading"><div className={styles.cardHeading}><span className="foundation-kicker">Danger zone</span><h3 id="account-danger-heading">Account actions</h3></div>{accountLifecycle.clearPredictionsAvailable ? <AccountRow title="Clear my predictions" description="Clear Original Predictor group scores and bracket picks before lock. KO Predictor, leagues and account details are untouched." action={<Button type="button" variant="danger" onClick={() => setClearConfirmOpen(true)} disabled={busy}>Clear predictions</Button>} /> : <AccountRow title="Clear my predictions" description="Once picks lock, this action is no longer available." action={<span className={styles.pill}>Locked</span>} disabled />}<AccountRow title="Sign out" description="Any guest picks on this device stay here." action={<Button type="button" variant="danger" onClick={() => setSignOutConfirmOpen(true)} disabled={busy}>Sign out</Button>} /></Card>
      <AuthNotice notice={notice} />
    </section>
  )
}
