...
export function normaliseAdminMatch(row) {
  return Object.freeze({
    ...
    fixtureCode: row.fixture_code ?? null,
    scheduledDate: row.scheduled_date ?? null,
    kickoffAt: normaliseTimestamp(row.kickoff_at),
    scheduleStatus: row.schedule_status ?? 'provisional',
    participantsStatus: row.participants_status ?? 'provisional',
    venueId: row.venue_id ?? null,
    venueName: row.venue_name ?? null,
    venueCity: row.venue_city ?? null,
    venueTimezone: row.venue_timezone ?? null,
    fixtureRevision: Number(row.fixture_revision ?? 1),
...
export function normaliseAdminControlRoom(value = {}) {
  ...
  return Object.freeze({
    ...
    health: Object.freeze({
      ...
      fixturesMissingDate: Number(health.fixtures_missing_date ?? 0),
      fixturesMissingVenue: Number(health.fixtures_missing_venue ?? 0),
      fixturesMissingConfirmedKickoff: Number(health.fixtures_missing_confirmed_kickoff ?? 0),
      provisionalScheduleFixtures: Number(health.provisional_schedule_fixtures ?? 0),
      officialDateVenueFixtures: Number(health.official_date_venue_fixtures ?? 0),
      officialDatetimeFixtures: Number(health.official_datetime_fixtures ?? 0),
      provisionalParticipantFixtures: Number(health.provisional_participant_fixtures ?? 0),
      confirmedParticipantFixtures: Number(health.confirmed_participant_fixtures ?? 0),
      ...
      pendingResults: Number(health.pending_results ?? 0),
      ...
      completedScoringRuns: Number(health.completed_scoring_runs ?? 0),
      failedScoringRuns: Number(health.failed_scoring_runs ?? 0),
      staleRunningScoringRuns: Number(health.stale_running_scoring_runs ?? 0),
      completeTeamProfiles: Number(health.complete_team_profiles ?? 0),
      incompleteTeamProfiles: Number(health.incomplete_team_profiles ?? 0),
    ...
    tournamentPicks: Object.freeze({
      contractVersion: value.tournament_picks?.contract_version ?? null,
      contractReady: Boolean(value.tournament_picks?.contract_ready),
      outcomeActivationReady: Boolean(value.tournament_picks?.outcome_activation_ready),
      activationDependency: value.tournament_picks?.activation_dependency ?? null,
    }),
