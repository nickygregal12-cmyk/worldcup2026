      ...
      {selectedMatch && draft && (
        <div className="foundation-admin-workspace">
          <article className="foundation-results-card foundation-results-card--wide">
            ...
            {validation && !validation.valid && <ul className="foundation-admin-validation">{validation.errors.map(error => <li key={error}>{error}</li>)}</ul>}
            <button type="button" disabled={!validation?.valid || !resultEntryEnabled || working} onClick={() => runAction(
          ...
          <article className="foundation-results-card">
            ...
            <button type="button" disabled={statusNote.trim().length < 5 || working} onClick={() => runAction(
          ...
          <article className="foundation-results-card">
            ...
            <button type="button" disabled={!scoringEnabled || selectedMatch.resultStatus !== 'confirmed' || recalculationNote.trim().length < 5 || working} onClick={() => runAction(
