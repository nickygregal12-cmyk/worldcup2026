...
function Action({ href, icon, title, copy, onClose, primary = false }) {
  return (
    <a className={`${styles.action} ${primary ? styles.primaryAction : ''}`.trim()} href={href} onClick={onClose}>
...
export default function PlayerQuickView({ player, competitionKey, onClose }) {
  ...
  const playerName = player?.isCurrentUser ? 'Your league profile' : 'League profile'
  const competitionName = competitionKey === LEAGUE_COMPETITION.ORIGINAL ? 'Original Predictor' : 'KO Predictor'
      ...
      {player && (
        <div className={styles.body}>
          ...
          <nav className={styles.actions} aria-label={`${player.displayName} profile options`}>
            <Action href={href('overview')} icon="account" title="View full profile" copy="Overview, released picks, tables and bracket health" onClose={onClose} primary />
          ...
          <p className={styles.privacy}><Icon name="lock" size={16} /> <span>Points already earned remain visible for fairness. Unreleased selections stay protected by the tournament lock.</span></p>
        </div>
      )}
    </Dialog>
  )
