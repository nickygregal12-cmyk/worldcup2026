...
uncommitted final-experience batch and still needs automated and visual proof. `ADOPTED` means the route
already has a compatible baseline but remains part of the final cross-route acceptance. `SCHEDULED` and
`GATED` are mandatory later work, not optional polish.

The sole visual/product-experience authority is
`docs/reference-prototypes/euro28-product-experience-final.md`. Earlier v3 and page-prototype
treatments are provenance. The route register schedules production adoption; it does not make mock
prototype data or markup eligible for production use.
...
| Groups `#/groups` | CURRENT REVIEW | Groups and qualification | Keep By group/By date, persisted choice, next-match landing and clickable Match Centre cards. Compact phone score rows retain both 48px arrow steppers and direct entry; the shared six-team third-place compound is implemented, while live/predicted switching and final visual approval remain. |
...
| Results `#/results` | CURRENT REVIEW | Results story | Phase-aware fixture priority, bounded next matches, progressive full-feed disclosure and shared group/third-place qualification are implemented. The real bracket appears only near group completion; richer league/bracket consequence storytelling and final visual approval remain. |
...
1. Final contract adoption and shared foundations — tokens, shell, flags, shared qualification compound and canonical data ownership.
2. Teams/Tournament and Groups — establish the shared football/qualification story before downstream pages consume it.
3. Bracket/Health, Leagues, Player Profile/H2H, Match Centre, Home/Results and supporting destinations in the production tracker order.
...
7. **Capacity decision** [GATE: signups]. The recorded initial public cap is
   50 users and 20 leagues before SMTP, with a post-SMTP target of 100 users and 20 leagues. Plan against the current low-cost/free setup first.
...
- OWNER-SIGNUP-DECISIONS-1 — CLOSED: the owner decisions are now recorded for Contact admin support wording, 50 users / 20 leagues before SMTP, 100 users / 20 leagues after SMTP review, current low-cost/free planning, email confirmation ON, conservative privacy wording, moderation policy and not staying invite-only after moderation is ready. Public registration still remains closed.
...
5A. STAGE-CORE-PAGE-ADOPTION-1A-GROUPS completed as the Groups slice under STAGE-CORE-PAGE-ADOPTION-1; STAGE-CORE-PAGE-ADOPTION-1
...
Accepted as a no-write preparation step before any staging seed executor. It records planned executor modules, blocked P6B command names, future P6C approval requirements, dual-marker teardown, zero-residue proof and reseed proof.
