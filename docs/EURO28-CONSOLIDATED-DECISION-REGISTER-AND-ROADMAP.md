...
**Stage DESIGN-CONTRACTS-APPROVED-ANCHORS-1** records the approved Groups and League visual contracts for the design-contract programme. `docs/reference-prototypes/euro28-groups-page-prototype.html` is the approved Groups Night Broadcast identity anchor and `docs/reference-prototypes/euro28-league-page-prototype.html` is the approved League visual contract. Neither surface may be redrafted unless Nicky explicitly reopens it. Bracket and KO Predictor Batch 1 files remain unapproved candidates only; they are not binding contracts and must not be implemented until Nicky selects a variant or approves a merge. This package is docs/reference-only with no `src/`, `supabase/`, test or migration change, no route/scoring/resolver/Auth/signup change, no Supabase write, no service-role credential use/read/print, active migrations remain 18 and no Migration 019 is created. Original Predictor and KO Predictor remain separate, and predicted/live bracket contexts must never blend.
...
Scope remains presentation-only: no scoring, resolver, Supabase write, service-role, route or migration change. Active migrations remain 18 with no Migration 019.
