...
Stage DESIGN-CONTRACTS-APPROVED-ANCHORS-1 is accepted as a docs/reference-only visual-anchor record. Groups is approved as the Night Broadcast identity anchor in `docs/reference-prototypes/euro28-groups-page-prototype.html`, and Leagues is approved as its own visual contract in `docs/reference-prototypes/euro28-league-page-prototype.html`. Neither Groups nor Leagues should be redrafted unless Nicky explicitly reopens the surface. Bracket and KO Predictor Batch 1 files remain candidate references only and are not approved or binding. No `src/`, `supabase/`, test or migration change is introduced; no route, scoring, resolver, Auth, signup, Supabase write or service-role credential use/read/print change is introduced. Original Predictor and KO Predictor remain separate, predicted/live bracket contexts never blend, active migrations remain 18 and no Migration 019 is created.
