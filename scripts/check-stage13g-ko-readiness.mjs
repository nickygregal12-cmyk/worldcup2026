...
const leagueCollectionsUi = read('src/leagues/LeagueCollectionTabs.jsx')
...
if (!leagueUi.includes('{koLeagueReady && (') || !leagueUi.includes('<LeagueCollectionTabs')) fail('League UI does not hide the KO collection switch before readiness')
if (!leagueCollectionsUi.includes('KO Predictor') || !leagueCollectionsUi.includes('Original league memberships do not carry across')) fail('League collection switch does not preserve independent KO membership')
...
if (!leagueAudit.includes('koReadiness={koReadiness}') || !leagueAudit.includes('LeagueCollectionTabs')) fail('League timing audit does not enforce shared KO-readiness adoption')
