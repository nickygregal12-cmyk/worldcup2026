...
const stageDoc = read('docs/STAGE-13G-B-PLAYER-TEAM-LIFECYCLE.md')
...
if (!playerModel.includes('Only selections released by the existing server privacy rules are shown')) fail('Player Insight did not preserve canonical server privacy copy')
if (!playerModel.includes('KO Predictor points never combine here')) fail('Player Insight Original/KO separation copy is missing')
...
if (!teamModel.includes('KO Predictor data is not included') || !teamModel.includes('no Original/KO points are combined')) fail('Team Profile separation copy is missing')
