...
const getRecentlyCompletedWindow = async () => {
  ...
  const { data: allMatches } = await supabase
    .from('matches')
    .select('id, match_number, kickoff_time, home_score, away_score, status, home_team:home_team_id(name, short_code, flag_emoji), away_team:away_team_id(name, short_code, flag_emoji)')
  ...
  return { yesterdayMatches, todayMatches, todayStart }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    profiles?.forEach(p => {
      ...
      pushEnabledMap[p.id] = p.push_enabled !== false
    })
    ...
    for (const sub of subscriptions) {
      ...
      const firstYesterdayKickoff = yesterdayMatches.length ? new Date(yesterdayMatches[0].kickoff_time) : null
      ...
      const missingToday = unlockedTodayMatches.filter(m => !userTodayPredicted.has(`${uid}:${m.id}`))
      ...
      if (hasYesterdayResults && !hasSeenResults) {
        ...
        body = hasMissingPicks
          ? `${missingToday.length} match${missingToday.length > 1 ? 'es' : ''} today still to predict`
          : yesterdayMatches.slice(0, 2).map(m => `${m.home_team?.flag_emoji}${m.home_team?.short_code} ${m.home_score}–${m.away_score} ${m.away_team?.short_code}${m.away_team?.flag_emoji}`).join(' · ')
      } else if (hasMissingPicks) {
        const names = missingToday.slice(0, 2).map(m => `${m.home_team?.flag_emoji}${m.home_team?.short_code} v ${m.away_team?.short_code}${m.away_team?.flag_emoji}`).join(', ')
      ...
      try {
        await webpush.sendNotification(
          ...
          JSON.stringify({ title, body, tag: `wc26-${todayStart.toISOString().split('T')[0]}`, url: '/predictions' })
        )
        sent++
      } catch (err) {
